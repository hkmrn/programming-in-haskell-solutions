...
int2let isUpperCase n 
    | isUpperCase = chr (ord 'A' + n)
    | otherwise = chr (ord 'a' + n)

shift :: Int -> Char -> Char 
shift n c 
    | isLower c = int2let False ((let2int c + n) `mod` 26)
    | isUpper c = int2let True ((let2int c + n) `mod` 26)
    | otherwise = c
